...
\begin{figure}
    ...
    \caption{Block diagram of the Xilinx\textsuperscript{\textregistered} Zynq\textsuperscript{\textregistered}-7000 All Programmable \ac{SoC}\todo{in 4.3 verschieben und darauf eingehen}}
    \label{fig:plat/base/zynq}
\end{figure}

\section{Xilinx ZYNQ-7000 SoC}
The hardware used is a ZedBoard\textsuperscript{TM} by Digilent\textsuperscript{\textregistered}. It is equipped with a Zynq\textsuperscript{\textregistered}-7000 All Programmable \ac{SoC} by Xilinx\textsuperscript{\textregistered} and natively supported by Pulpissimo. The Zynq-7000 processor features a \ac{PL} as well as a Dual ARM\textsuperscript{\textregistered} Cortex\textsuperscript{\textregistered}-A9 MPCore\textsuperscript{TM} hard IP able to run Linux. The hard IP and \ac{PL} are connected via an AXI bus which can be configured using block diagrams inside the Vivado\textsuperscript{\textregistered} design environment. \cite{Zynq7000SoC} An architectural block diagram can be seen in figure \ref{fig:plat/base/zynq}. \todo{trademarks weg}
