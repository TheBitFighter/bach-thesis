...
An \ac{FPU} instruction is recorded when the \ac{APU}'s enable signal is raised by the execution stage and the decode stage marks the instruction as valid.
...
\begin{table}
    ...
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{lp{0.087\textwidth}p{0.18\textwidth}p{0.6\textwidth}}
        \hline \\[-0.9em]
        \textbf{Counter}    & \textbf{GPIO} & \textbf{Memory}   & \textbf{Description}  \\
                            & \textbf{unit \#} & \textbf{address} & \\
        \hline \\[-0.9em]
